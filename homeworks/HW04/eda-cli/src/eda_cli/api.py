...
class QualityFlagsResponse(BaseModel):
    """Ответ заглушки, флаги качества датасета."""
...
class SampleResponse(BaseModel):
    """Ответ заглушки, несколько случайных строк датасета."""

    sample: str | None = Field(
        default=None,
        description="Несколько (n) случайных строк датасета",
    )
...
# ---------- /quality-flags-from-csv: флаги качества по CSV-файлу через EDA-логику ----------
...
# ---------- /sample: случайные строки из CSV-файла через EDA-логику ----------
...
@app.post(
    ...
    tags=["dataset"],
    summary="Несколько случайных строк из CSV-файла",
)
async def dataset_sample(file: UploadFile = File(...), n = 5) -> SampleResponse:
    
    # Эндпоинт, который принимает CSV-файл и количество строк (n) и возвращает случайные строки из CSV-файла.
    ...
    sample_data = df.sample(int(n)).to_string(index=False)
