...
def report(
    ...
    min_missing_share: float = typer.Option("0.05", help="Порог доли пропусков проблемной колонки."),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Присутствуют колонки с одинаковыми значениями: **{quality_flags['has_constant_columns']}**\n")
        f.write(f"- Все идентификаторы (user_id) уникальны: **{quality_flags['has_suspicious_id_duplicates']}**\n")
        if quality_flags['high_missing_columns']:
            f.write(f"- Проблемные колонки: **{quality_flags['high_missing_columns']}**\n\n")
        else:
            f.write(f"- Проблемных колонок нет.\n\n")
        ...
        if not top_cats:
            ...
            f.write(f"- Количество top-значений для категориальных признаков: **{top_k_categories}**.\n\n")
...
def sample(
    ...
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
    count: int = typer.Option(4, help="Количество случайных строк."),
) -> None:
    """
    Напечатать несколько случайных строк датасета
    """
    df = _load_csv(Path(path), sep=sep, encoding=encoding)

    typer.echo(f"\nКоличество выведенных строк: {count}.")
    typer.echo(df.sample(count).to_string(index=False))
